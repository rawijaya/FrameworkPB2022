...
gnv_sys = Create n_cst_sys
//cbx_htmlformat.enabled = false
end event
...
event clicked;String ls_To, ls_Cc, ls_Bcc, ls_ToArray[], ls_CcArray[], ls_BccArray[]
String ls_Subject,ls_Body, ls_resourcefile
Integer i, li_Return, li_filenum, li_proxytype
String ls_Log,ls_htmlstring
...
//Get Recipients 
ls_To = sle_to.Text 
ls_To = gnv_sys.of_ReplaceAll(ls_To,' ', '',False)
ls_To = gnv_sys.of_ReplaceAll(ls_To,',', ';',False)
gnv_sys.of_StringToArray(ls_To,';', ls_ToArray[])

ls_Cc = sle_cc.Text 
ls_Cc = gnv_sys.of_ReplaceAll(ls_Cc,' ', '',False)
ls_Cc = gnv_sys.of_ReplaceAll(ls_Cc,',', ';',False)
gnv_sys.of_StringToArray(ls_Cc,';', ls_CCArray[])

ls_Bcc = sle_bcc.Text 
ls_Bcc = gnv_sys.of_ReplaceAll(ls_Bcc,' ', '',False)
ls_Bcc = gnv_sys.of_ReplaceAll(ls_Bcc,',', ';',False)
gnv_sys.of_StringToArray(ls_Bcc,';', ls_BccArray[])
...
//	ismtp_email.message.HTMLBody = mle_Body.Text
//	
//	If Pos(Lower(mle_body.Text), Lower(is_ContantID)) > 0 then
//		ismtp_email.message.AddLinkedResource(gs_currentdir + "\History.png" ,is_ContantID )
//	End If
//	
//Else
...
gs_CurrentDIR = GetCurrentDirectory( )
	If rte_Body.SaveDocument (gs_CurrentDIR + "\temp.html" ,FileTypeHTML! ) = 1 Then
		li_FileNum = FileOpen( gs_CurrentDIR + "\temp.html",TextMode!,Read!)
...
event clicked;If This.Checked Then
	...
	End If
Else
	sle_Subject.text = gnv_sys.of_ReplaceAll(sle_Subject.text,' (Async)','',False)
End If
end event
...
Else
	...
	sle_Subject.text = gnv_sys.of_ReplaceAll(sle_Subject.text,' (HTML Format)','',False)
...
event clicked;gs_CurrentDIR = GetCurrentDirectory( )
...
event clicked;Openwithparm(w_emailsettings,"")
end event
